...
public class TutorialLevel : PlayableLevel 
{
	...
	public override void setFocusLevel (float focusLevel)
	{
		...
		mPlayButton.transform.localPosition += GUICanvasMenu.Instance.MenuGUICanvas().PlayButton().PositionOffset();
		mPlayButton.transform.localScale = Vector3.one * 0.84f * GUICanvasMenu.Instance.MenuGUICanvas().PlayButton().ScaleFactor();
